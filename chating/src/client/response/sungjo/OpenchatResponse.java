...
//				Long groupid  = chatcontent.get(0).getGroupid();
				...
				for(Chat content : chatcontent) {
					...
					clientback.getChatMap().get(groupid).appendMSG(content.getUserid() + " : " + content.getContent() + "   ---" + content.getCount() + "\n");
				}
			}else {
				ArrayList<Chat> list = clientback.getChatFileListMap().get(groupid);
				oos = clientback.getChatFileMap().get(groupid);
				chatwindow = chatMap.get(groupid);
				for(Chat content : list) {
					oos.writeObject(content);
					clientback.getChatMap().get(groupid).appendMSG(content.getUserid() + " : " + content.getContent() + "   ---" + content.getCount() + "\n");
				}
			}
